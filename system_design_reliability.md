...
* **Shopify REST & GraphQL APIs:** 
  * **REST API** is used to fetch specific product metafields.
  * **GraphQL API** is used for batch querying orders via tag filtering (`tag:reminder_YYYY-MM-DD`) and performing mutations (adding/removing tags). GraphQL is chosen for its efficiency, allowing us to retrieve multiple orders, line items, and customer data in a single payload, reducing Shopify API rate-limit consumption.
* **JavaScript (n8n Code Node):** Used for custom date calculations. Standardizing on JS allows for clean type checking (e.g., parsing integers) and fallback default handling (e.g., default 30-day lifespan) that is easily readable and maintainable compared to complex n8n expression syntax.
...
[n8n Workflow Part A]
       ...
       ├──► JS: Calculate (Lifespan - 5 Days)
       ├──► Write order metafield "reminder_date" (REST)
       └──► Tag Order "reminder_YYYY-MM-DD" (GraphQL)
...
[n8n Workflow Part B]
       │
       ├──► Fetch all orders tagged "reminder_today" (GraphQL)
       ├──► Split Out into individual threads
       ├──► Send WhatsApp message via Messaging.digital API
       └──► Untag Order "reminder_today" (GraphQL)
```
...
* **Impact:** The `reminder_date` is never calculated, and the order is never tagged. The customer will not receive a reminder.
...
* **Mitigation (Implemented):** The JavaScript code includes a resilient fallback block:
  ```javascript
  if (!lifespanDays) lifespanDays = 30; // Default to 30 days
  ```
* **Diagnosis:** Monitor executions of the JavaScript node in n8n. If the output payload uses exactly 30 days, check if the product in Shopify has `custom.lifespan_days` set.
...
* **What happens:** The WhatsApp message is successfully sent, but the Shopify GraphQL mutation to remove `reminder_YYYY-MM-DD` fails (e.g., Shopify API timeout).
* **Impact:** The order remains tagged `reminder_YYYY-MM-DD`. On the next daily run, the workflow will fetch the same order again and send a **duplicate WhatsApp message** to the customer.
* **Mitigation:** 
  1. Configure n8n's GraphQL untag node to retry on failure.
  2. Implement an validation check: log execution IDs in a simple KV database (or Google Sheet) to prevent duplicate runs for the same order.
...
3. **Data Sanitization:** Phone numbers are cleaned programmatically inside n8n to strip any formatting, spaces, or country-code symbols (e.g., `+`, `-`, `()`) before calling the WhatsApp API:
   ```javascript
   {{ ($json.node.phone || $json.node.customer?.phone || '').replace(/\D/g, '') }}
   ```
4. **Fallback Chain:** Checks `order.phone` first, falling back to `customer.phone` to guarantee maximum deliverability.
